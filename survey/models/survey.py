...
def dict_keys_startswith(dictionary, string):
    ...
    matched_keys = [key for key in dictionary.keys()
...
class SurveyStage(models.Model):
    ...
    closed = \
        fields.Boolean(help="If closed, people won't be able to answer to surveys in this column."
                       )
    fold = fields.Boolean(string='Folded in kanban view')

    _sql_constraints = [('positive_sequence', 'CHECK(sequence >= 0)',
...
class Survey(models.Model):
    ...
    page_ids = fields.One2many('survey.page', 'survey_id',
    ...
    tot_sent_survey = fields.Integer('Number of sent surveys',
            compute='_compute_survey_statistic')
    tot_start_survey = fields.Integer('Number of started surveys',
            compute='_compute_survey_statistic')
    tot_comp_survey = fields.Integer('Number of completed surveys',
            compute='_compute_survey_statistic')
    ...
    email_template_id = fields.Many2one('mail.template',
            string='Email Template', ondelete='set null')
    thank_you_message = fields.Html('Thanks Message', translate=True,
    ...
    def _compute_survey_statistic(self):
        ...
        sent_survey = UserInput.search([('survey_id', 'in', self.ids),
                ('type', '=', 'link')])
        start_survey = UserInput.search(['&', ('survey_id', 'in',
                self.ids), '|', ('state', '=', 'skip'), ('state', '=',
                'done')])
        complete_survey = UserInput.search([('survey_id', 'in',
                self.ids), ('state', '=', 'done')])

        for survey in self:
            survey.tot_sent_survey = \
                len(sent_survey.filtered(lambda user_input: \
                    user_input.survey_id == survey))
            survey.tot_start_survey = \
                len(start_survey.filtered(lambda user_input: \
                    user_input.survey_id == survey))
            survey.tot_comp_survey = \
                len(complete_survey.filtered(lambda user_input: \
                    user_input.survey_id == survey))

    def _compute_survey_url(self):
        """ Computes a public URL for the survey """

        base_url = ('/' if self.env.context.get('relative_url'
                    ) else self.env['ir.config_parameter'
                    ].get_param('web.base.url'))
        for survey in self:
            survey.public_url = urljoin(base_url, 'survey/start/%s'
                    % slug(survey))
            survey.print_url = urljoin(base_url, 'survey/print/%s'
                    % slug(survey))
            survey.result_url = urljoin(base_url, 'survey/results/%s'
                    % slug(survey))
            survey.public_url_html = '<a href="%s">%s</a>' \
                % (survey.public_url, _('Click here to start survey'))
    ...
    def next_page(
        ...
        current_page_index = pages.index(filter(lambda p: p[1].id \
                == page_id, pages)[0])
    ...
    def filter_input_ids(self, filters, finished=False):
        ...
        if filters:
            ...
            for current_filter in filters:
                (row_id, answer_id) = (current_filter['row_id'],
                        current_filter['answer_id'])
                if row_id == 0:
                    choice.append(answer_id)
                else:
                    domain_filter.extend(['|', ('value_suggested_row.id'
                            , '=', row_id), ('value_suggested.id', '=',
                            answer_id)])
            if choice:
                domain_filter.insert(0, ('value_suggested.id', 'in',
                        choice))
            else:
                domain_filter = domain_filter[1:]
            input_lines = self.env['survey.user_input_line'
                                   ].search(domain_filter)
            filtered_input_ids = [input_line.user_input_id.id
        ...
        if finished:
            ...
            if not filtered_input_ids:
                user_inputs = UserInput.search([('survey_id', '=',
                        self.id)])
            else:
                user_inputs = UserInput.browse(filtered_input_ids)
            return user_inputs.filtered(lambda input_item: \
                    input_item.state == 'done').ids
    ...
    def prepare_result(self, question, current_filters=None):
        ...
        if question.type in ['simple_choice', 'multiple_choice']:
            answers = {}
            comments = []
            [answers.update({label.id: {'text': label.value,
             'count': 0, 'answer_id': label.id}}) for label in
             question.labels_ids]
            for input_line in question.user_input_line_ids:
                if input_line.answer_type == 'suggestion' \
                    and answers.get(input_line.value_suggested.id) \
                    and (not current_filters
                         or input_line.user_input_id.id
                         in current_filters):
                    answers[input_line.value_suggested.id]['count'] += 1
                if input_line.answer_type == 'text' \
                    and (not current_filters
                         or input_line.user_input_id.id
                         in current_filters):
                    comments.append(input_line)
            result_summary = {'answers': answers.values(),
                              'comments': comments}
        ...
        if question.type == 'matrix':
            ...
            res = dict()
            comments = []
            [rows.update({label.id: label.value}) for label in
             question.labels_ids_2]
            [answers.update({label.id: label.value}) for label in
             question.labels_ids]
            ...
            for input_line in question.user_input_line_ids:
                if input_line.answer_type == 'suggestion' \
                    and (not current_filters
                         or input_line.user_input_id.id
                         in current_filters) \
                    and input_line.value_suggested_row:
        ...
        if question.type in ['free_text', 'textbox', 'datetime']:
            result_summary = []
            for input_line in question.user_input_line_ids:
                if not current_filters or input_line.user_input_id.id \
                    in current_filters:
        ...
        if question.type == 'numerical_box':
            ...
            if all_inputs:
                result_summary.update({
                    'average': round(sum(all_inputs) / len(all_inputs),
                            2),
    ...
    def get_input_summary(self, question, current_filters=None):
        ...
        if question.survey_id.user_input_ids:
            total_input_ids = current_filters or [input_id.id
                    for input_id in question.survey_id.user_input_ids
            ...
            result['answered'] = len(set(question_input_ids)
                    & set(total_input_ids))
            result['skipped'] = result['total_inputs'] \
                - result['answered']
        return result
    ...
    def action_send_survey(self):
        ...
        template = self.env.ref('survey.email_template_survey',
...
class SurveyQuestion(models.Model):
    ...
    description = fields.Html('Description',
                              help="Use this field to add \
        additional explanations about your question"
                              , translate=True,
    ...
    type = fields.Selection([
        ...
        ('matrix', 'Matrix'),
        ], string='Type of Question', default='free_text',
            required=True)
    ...
    labels_ids = fields.One2many('survey.label', 'question_id',
                                 string='Types of answers',
                                 oldname='answer_choice_ids', copy=True)
    labels_ids_2 = fields.One2many('survey.label', 'question_id_2',
    ...
    column_nb = fields.Selection([('12', '1'), ('6', '2'), ('4', '3'),
                                 ('3', '4'), ('2', '6')],
    ...
    comment_count_as_answer = \
        fields.Boolean('Comment Field is an Answer Choice',
    ...
    validation_error_msg = fields.Char('Validation Error message',
            oldname='validation_valid_err_msg', translate=True,
            default=lambda self: \
            _('The answer you entered has an invalid format.'))
    ...
    constr_error_msg = fields.Char('Error message',
                                   ...
                                   translate=True, default=lambda self: \
                                   _('This question requires an answer.'
                                   ))
    user_input_line_ids = fields.One2many('survey.user_input_line',
            'question_id', string='Answers', domain=[('skipped', '=',
            False)])

    _sql_constraints = [('positive_len_min',
    ...
    def validate_datetime(self, post, answer_tag):
        ...
        if answer and self.validation_required:
            ...
            try:
                ...
                min_date = \
                    datetime_from_string(self.validation_min_date)
                max_date = \
                    datetime_from_string(self.validation_max_date)
                ...
                if min_date and max_date and not min_date <= dateanswer \
                    ...
                    errors.update({answer_tag: self.validation_error_msg})
                                ...
                                # check that it is a datetime has been done hereunder
    ...
    def validate_simple_choice(self, post, answer_tag):
        ...
        if self.constr_mandatory and answer_tag not in post:
            errors.update({answer_tag: self.constr_error_msg})
        if self.constr_mandatory and answer_tag in post \
            and not post[answer_tag].strip():
            errors.update({answer_tag: self.constr_error_msg})

        # Answer is a comment and is empty

        if self.constr_mandatory and answer_tag in post \
            and post[answer_tag] == '-1' \
            and self.comment_count_as_answer and comment_tag in post \
            and not post[comment_tag].strip():
            errors.update({answer_tag: self.constr_error_msg})
        return errors
    ...
    def validate_multiple_choice(self, post, answer_tag):
        ...
        if self.constr_mandatory:
            ...
            comment_flag = answer_candidates.pop('%s_%s' % (answer_tag,
                    -1), None)
            if self.comments_allowed:
                comment_answer = answer_candidates.pop('%s_%s'
                        % (answer_tag, 'comment'), '').strip()

            # Preventing answers with blank value

            if all([(True if not answer.strip() else False)
                   for answer in answer_candidates.values()]) \
                and answer_candidates:
                errors.update({answer_tag: self.constr_error_msg})

            # There is no answer neither comments (if comments count as answer)

            if not answer_candidates and self.comment_count_as_answer \
                and (not comment_flag or not comment_answer):
                errors.update({answer_tag: self.constr_error_msg})

            # There is no answer at all

            if not answer_candidates \
                and not self.comment_count_as_answer:
                errors.update({answer_tag: self.constr_error_msg})
        return errors
    ...
    def validate_matrix(self, post, answer_tag):
        ...
        if self.constr_mandatory:
            ...
            if self.matrix_subtype == 'simple':
                ...
                answer_number = len(set([sk.rsplit('_', 1)[0] for sk in
                                    answer_candidates.keys()]))
            else:
...
class SurveyLabel(models.Model):
    ...
    question_id = fields.Many2one('survey.question', string='Question',
                                  ondelete='cascade')
    question_id_2 = fields.Many2one('survey.question',
                                    string='Question 2',
                                    ondelete='cascade')
...
class SurveyUserInput(models.Model):
    ...
    survey_id = fields.Many2one('survey.survey', string='Survey',
                                ...
                                ondelete='restrict')
    date_create = fields.Datetime('Creation Date',
    ...
    state = fields.Selection([('new', 'Not started yet'), ('skip',
                             'Partially completed'), ('done',
                             'Completed')], string='Status',
                             default='new', readonly=True)
    test_entry = fields.Boolean(readonly=True)
    token = fields.Char('Identification token', default=lambda self: \
                        str(uuid.uuid4()), readonly=True,
    ...
    result_url = fields.Char('Public link to the survey results',
                             related='survey_id.result_url')
    print_url = fields.Char('Public link to the empty survey',
                            related='survey_id.print_url')

    quizz_score = fields.Float('Score for the quiz',
    ...
    def _compute_quizz_score(self):
        for user_input in self:
            user_input.quizz_score = \
                sum(user_input.user_input_line_ids.mapped('quizz_mark'))

    _sql_constraints = [('unique_token', 'UNIQUE (token)',
                        'A token must be unique!'),
                        ('deadline_in_the_past',
                        'CHECK (deadline >= date_create)',
...
class SurveyUserInputLine(models.Model):
    ...
    date_create = fields.Datetime('Create Date',
                                  default=fields.Datetime.now,
                                  required=True)
    skipped = fields.Boolean('Skipped')
    answer_type = fields.Selection([('text', 'Text'), ('number',
                                   'Number'), ('date', 'Date'),
                                   ('free_text', 'Free Text'),
    ...
    value_suggested_row = fields.Many2one('survey.label',
            string='Row answer')
    ...
    def _check_answer_type(self):
        for uil in self:
            fields_type = {
                ...
                'number': bool(uil.value_number) or uil.value_number \
                    == 0,
