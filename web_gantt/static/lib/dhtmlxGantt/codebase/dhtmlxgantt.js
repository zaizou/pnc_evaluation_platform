    ...
    if (this.contextMenu.isDhtmlxMenuObject) {
        ...
        if (_isIE) {
            var dEl0 = window.document.documentElement,
                dEl1 = window.document.body,
                corrector = new Array((dEl0.scrollLeft || dEl1.scrollLeft), (dEl0.scrollTop || dEl1.scrollTop));
...
GanttProject.prototype.addDayInPanelTime = function() {
    ...
    var table = this.Chart.panelTime.firstChild,
...
GanttTask.prototype.moveItem = function(event) {
    ...
    if (this.checkMove) {
        ...
        if (res && ((this.minPosXMove <= posTaskItem)) && ((posTaskItemR <= this.maxPosXMove) || (this.maxPosXMove == -1))) {
...
GanttTask.prototype.getDescStr = function() {
    ...
    for (var i = 0; i < this.Chart.paramShowTask.length; i++) {
        var prop = this.Chart.paramShowTask[i],
            propValue = this.TaskInfo[prop];
        switch (prop) {
            case "Name":
                if (str != "") str += delim;
                str += propValue;
    ...
    if (this.Chart.isShowConMenu) {
        var showContMenu = function(event) {
            ...
            var hideContMenu = function() {
                self.Chart.contextMenu.hideContextMenu();
                if (self.Chart._isIE) self.Chart.content.detachEvent("mousedown", hideContMenu);
                else
                    self.Chart.content.removeEventListener("mousedown", hideContMenu, false)
            };
            self.Chart.content.onmousedown = hideContMenu;
