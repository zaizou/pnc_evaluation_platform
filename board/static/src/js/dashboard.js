        ...
        on_load_action: function(result, index, action_attrs) {
            ...
            if (view_mode && view_mode != action.view_mode) {
                action.views = _.map(view_mode.split(','), function(mode) {
                    ...
                    return _(action.views).find(function(view) { return view[1] == mode; }) || [false, mode];
                });
            }
    ...
    FavoriteMenu.include({
        ...
        add_dashboard: function() {
            ...
            var d = pyeval.eval('domain', domain),
                board = new Model('board.boardone'),
                name = self.$add_dashboard_input.val();
            var borad_name = name.split("-")[1];
            name = name.split("-")[0];
            if (typeof borad_name == 'undefined')
                borad_name = "";
