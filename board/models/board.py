...
class BoardGlobal(models.AbstractModel):
    _name = 'board.boardglo'
    _description = "Boardglo"
    _auto = False
    ...
    def fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
        ...
        res = super(BoardGlobal, self).fields_view_get(view_id=view_id, view_type=view_type, toolbar=toolbar, submenu=submenu)

        

        custom_view = self.env['ir.ui.view.customglo'].search([('user_id', '=', self.env.uid), ('ref_id', '=', view_id)], limit=1)
        _logger.warning("res Board global")
        _logger.warning(res)
...
class custom_view_glo(models.Model):
    _inherit = 'ir.ui.view.custom'
    _name = 'ir.ui.view.customglo'
